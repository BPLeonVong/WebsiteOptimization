...
var resizePizzas = function(size) {
    var newPizzaSize = document.getElementById("pizzaSize").innerHTML;
    window.performance.mark("mark_start_resize"); // User Timing API function

    /*A function that changes the value for the size of the pizza above the slider*/

    function changeSliderLabel(size) {
        //For some reason the innerHTML doesn't change here.
        switch (size) {
            case "1":
                newPizzaSize = "Small";
                return;
    ...
    //For some reason the function changeSliderLabel does not change the innerHTML, however still maintain its value, therefore do a quick change to display the new pizza size
    document.getElementById("pizzaSize").innerHTML = newPizzaSize;


    /*A function that returns the current size of the pizzas*/
    ...
    /*
    	A function to change the sizes of all the pizzas to the new size
    	It will make a variable that holds all the elements named randomPizzaContainer assign them
    	to the new width called from the GetPizzaSize function
    */
    function changePizzaSizes() {
        var AllThePizzas = document.getElementsByClassName("randomPizzaContainer");
        var newWidth = GetPizzaSize();
        var arrayLength = AllThePizzas.length;
...
function updatePositions() {
    ...
    var itemLeftArray = [];
    var phase;
    //Made a variable to hold the y position of the page, instead of calling document.body.scrollTop for each item.
    var scrollPositionY = document.body.scrollTop / 1250;

    //Find all the positions of where pizza has to be when changed
    for (var i = 0; i < items.length; i++) {
        phase = Math.sin((scrollPositionY) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
    //Calculates how many pizzas columns can be visible on screen
    var cols = Math.round(window.innerWidth / 256);
    var s = 256;
    var elem;
    //Calculates how many pizzas rows can be visible on screen
    var rows = Math.round(window.innerHeight / 256);

    //Iterates and creates column number of pizzas multiplied by row number of pizzas
...
//Globally scoped variable so I do not always have to make a new variable array for moving pizzas when I scroll the screen
